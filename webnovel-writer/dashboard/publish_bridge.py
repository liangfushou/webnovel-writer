...
_scripts_root_path = _package_root / "scripts"
_data_modules_root_path = _scripts_root_path / "data_modules"
for path in (_scripts_root_path, _data_modules_root_path):
    entry = str(path)
...
def setup_browser() -> str:
    ...
    task.message = "正在打开登录浏览器…"
    _task_store.add_log(task_id, "将打开番茄作家后台登录浏览器，请在弹出的窗口中手动完成登录。")

    def _run():
        try:
            exit_code = _run_async(cmd_setup_browser())
            if exit_code == 0:
                _task_store.update(
                    task_id,
                    ...
                    progress=1,
                    total=1,
                    message="登录配置完成，登录态已保存在本机。",
                )
                _task_store.add_log(task_id, "登录配置完成。")
            else:
                _task_store.update(
                    task_id,
                    status=TaskStatus.FAILED,
                    message="登录配置未完成，请重新尝试。",
                )
                _task_store.add_log(task_id, "登录配置未完成。")
        except Exception as e:
            _task_store.update(task_id, status=TaskStatus.FAILED, message=f"登录配置失败: {e}")
